...
ALTER TABLE "Trip"
  ADD COLUMN IF NOT EXISTS "estimatedFuelCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "estimatedMaintenanceCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "estimatedTripCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "estimatedProfitInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "estimatedProfitMarginPercent" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "profitabilityEstimatedAt" TIMESTAMP(3),
  ADD COLUMN IF NOT EXISTS "actualFuelCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualMaintenanceCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualExpenseCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualDriverPayoutInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualTollCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualTripCostInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualProfitInr" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "actualProfitMarginPercent" DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS "profitabilityFinalizedAt" TIMESTAMP(3);
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'NotificationType') THEN
    CREATE TYPE "NotificationType" AS ENUM ('TRIP_CREATED','TRIP_DISPATCHED','TRIP_STARTED','TRIP_COMPLETED','TRIP_CANCELLED');
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS "Notification_tripId_idx" ON "Notification"("tripId");
DO $$
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Notification_tripId_fkey') THEN
