...
const fleetAnalytics_1 = require("./fleetAnalytics");
...
(0, vitest_1.describe)('calculateFleetAnalytics', () => {
    (0, vitest_1.it)('calculates realized fleet performance without counting draft revenue', () => {
        const result = (0, fleetAnalytics_1.calculateFleetAnalytics)({ vehicles, fuel, maintenance, expenses, trips, now });
        (0, vitest_1.expect)(result.summary).toMatchObject({ realizedRevenue: 125000, operationalCost: 22000, realizedProfit: 103000, profitMargin: 82.4, costPerKm: 44, fuelEfficiency: 5, completedTrips: 2, totalDistanceKm: 500, fleetUtilization: 50 });
        (0, vitest_1.expect)(result.costBreakdown).toEqual({ fuel: 12000, maintenance: 5000, tolls: 1500, driverPayments: 2500, otherExpenses: 1000 });
    });
    (0, vitest_1.it)('returns per-vehicle profitability and six chronological monthly buckets', () => {
        const result = (0, fleetAnalytics_1.calculateFleetAnalytics)({ vehicles, fuel, maintenance, expenses, trips, now });
        (0, vitest_1.expect)(result.byVehicle.find(row => row.id === 'v1')).toMatchObject({ completedTrips: 1, totalTrips: 2, revenue: 100000, operationalCost: 19000, profit: 81000, marginPercent: 81, costPerKm: 47.5, roi: 8.1 });
        (0, vitest_1.expect)(result.monthlyTrend).toHaveLength(6);
        (0, vitest_1.expect)(result.monthlyTrend.slice(-2)).toEqual([
    ...
    (0, vitest_1.it)('handles an empty fleet without NaN or misleading percentages', () => {
        const result = (0, fleetAnalytics_1.calculateFleetAnalytics)({ vehicles: [], fuel: [], maintenance: [], expenses: [], trips: [], now });
        (0, vitest_1.expect)(result.summary).toMatchObject({ fuelEfficiency: 0, fleetUtilization: 0, operationalCost: 0, vehicleRoi: 0, realizedRevenue: 0, realizedProfit: 0, profitMargin: null, costPerKm: null });
        (0, vitest_1.expect)(result.byVehicle).toEqual([]);
    });
});
