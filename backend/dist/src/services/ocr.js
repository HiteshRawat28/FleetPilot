...
exports.extractDrivingLicense = extractDrivingLicense;
exports.extractOdometer = extractOdometer;
exports.parseReceiptText = parseReceiptText;
exports.extractReceipt = extractReceipt;
const tesseract_js_1 = require("tesseract.js");
function normalizedText(value) { return value.toUpperCase().replace(/[|]/g, 'I').replace(/\s+/g, ' ').trim(); }
...
async function extractDrivingLicense(buffer) {
...
async function extractOdometer(buffer) {
    const result = await (0, tesseract_js_1.recognize)(buffer, 'eng');
    const rawText = result.data.text || '';
...
async function extractReceipt(buffer) {
    const result = await (0, tesseract_js_1.recognize)(buffer, 'eng');
    return parseReceiptText(result.data.text || '', result.data.confidence || 0);
}
