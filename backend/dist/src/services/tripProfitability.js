...
exports.loadTripProfitabilityConfig = loadTripProfitabilityConfig;
exports.resolveFuelRate = resolveFuelRate;
...
function configuredNumber(value, fallback, name) {
...
function loadTripProfitabilityConfig(env = process.env) {
    return {
        fuelRatesInrPerKm: {
...
function resolveFuelRate(vehicleType, config) {
    const normalized = vehicleType.trim().toLowerCase();
    const source = /\b(bus|coach)\b/.test(normalized)
        ? 'BUS'
...
function resolvePredictedFuelRate(input) {
    const prediction = input.fuelPrediction;
    if (prediction) {
        if (!Number.isFinite(prediction.pricePerLitreInr) || prediction.pricePerLitreInr <= 0)
            throw new RangeError('fuel price must be a finite positive number');
        if (!Number.isFinite(prediction.efficiencyKmPerLitre) || prediction.efficiencyKmPerLitre <= 0)
            throw new RangeError('fuel efficiency must be a finite positive number');
        if (!prediction.priceAsOf || Number.isNaN(Date.parse(prediction.priceAsOf)))
            throw new RangeError('fuel price date must be valid');
        return {
            source: 'RECENT_FUEL_AND_TRIP_HISTORY',
...
function buildFuelPrediction(fuelLogs, completedTrips) {
    ...
    if (!validLogs.length || !validTrips.length)
        return undefined;
...
function calculateTripProfitability(input) {
    assertFiniteNonNegative(input.revenueInr, 'revenueInr');
    if (input.estimatedTollsInr !== null)
        assertFiniteNonNegative(input.estimatedTollsInr, 'estimatedTollsInr');
    ...
    if (!Number.isFinite(input.plannedDistanceKm) || input.plannedDistanceKm <= 0)
        throw new RangeError('plannedDistanceKm must be a finite positive number');
    Object.entries(input.config.fuelRatesInrPerKm).forEach(([name, value]) => {
        if (!Number.isFinite(value) || value <= 0)
            throw new RangeError(`fuel rate ${name} must be a finite positive number`);
    });
    if (!Number.isFinite(input.config.maintenanceMinHistoryKm) || input.config.maintenanceMinHistoryKm <= 0)
        throw new RangeError('maintenanceMinHistoryKm must be a finite positive number');
    if (!Number.isFinite(input.config.vehicleUsefulLifeKm) || input.config.vehicleUsefulLifeKm <= 0)
        throw new RangeError('vehicleUsefulLifeKm must be a finite positive number');
    ...
    const maintenanceRateSource = useHistory ? 'HISTORICAL_MAINTENANCE' : 'DEPRECIATION_HEURISTIC';
