...
async function expenseResponse(expense) {
    ...
    return { ...safeExpense, receiptUrl: receiptKey && (0, objectStorage_1.objectStorageConfigured)() ? await (0, objectStorage_1.signedObjectUrl)(receiptKey, 900) : null };
}
...
app.post('/api/driver/me/onboarding', driverOnly, driverDocumentUpload.fields([{ name: 'profilePhoto', maxCount: 1 }, { name: 'licenseFront', maxCount: 1 }, { name: 'licenseBack', maxCount: 1 }]), asyncRoute(async (req, res) => {
    ...
    await db.driver.update({ where: { id: req.user.driverId }, data: { onboardingStatus: client_1.OnboardingStatus.PENDING, reviewNote: null } });
    let ocr;
    try {
...
app.post('/api/driver/me/onboarding/confirm', driverOnly, asyncRoute(async (req, res) => {
    ...
    await db.$transaction([
        db.driver.update({ where: { id: req.user.driverId }, data: { ...data, onboardingStatus: client_1.OnboardingStatus.NEEDS_REVIEW, status: client_1.DriverStatus.OFF_DUTY, reviewNote: null } }),
...
app.post('/api/driver/me/expenses/receipt', driverOnly, driverReceiptUpload.single('receipt'), asyncRoute(async (req, res) => {
    ...
    if (!driver || driver.onboardingStatus !== client_1.OnboardingStatus.VERIFIED)
    ...
    const receiptKey = await (0, objectStorage_1.uploadPrivateObject)({ organizationId: req.user.organizationId, folder: `drivers/${driver.id}/receipts`, originalName: req.file.originalname, mimeType, buffer: req.file.buffer });
    const description = [fields.description, ocr.vendor, `Submitted by ${driver.name}`].filter(Boolean).join(' · ');
    const expense = await db.expense.create({ data: { organizationId: req.user.organizationId, vehicleId: trip.vehicleId, submittedByDriverId: driver.id, type: (fields.type || ocr.expenseType), description: description || undefined, amount: ocr.amount, date: ocr.date ? new Date(ocr.date) : new Date(), receiptKey, receiptOriginalName: req.file.originalname, receiptMimeType: mimeType, ocrConfidence: ocr.confidence }, include: { vehicle: true, submittedByDriver: { select: { id: true, name: true } } } });
...
app.post('/api/drivers/:id/onboarding/approve', allow(client_1.Role.FLEET_MANAGER, client_1.Role.SAFETY_OFFICER), asyncRoute(async (req, res) => {
    ...
    if (!driver)
        throw Object.assign(new Error('Driver not found'), { status: 404 });
    if (driver.onboardingStatus !== client_1.OnboardingStatus.NEEDS_REVIEW)
    ...
    res.json(await db.driver.update({ where: { id: driver.id }, data: { onboardingStatus: client_1.OnboardingStatus.VERIFIED, status: client_1.DriverStatus.AVAILABLE, reviewNote: null } }));
}));
app.post('/api/drivers/:id/onboarding/reject', allow(client_1.Role.FLEET_MANAGER, client_1.Role.SAFETY_OFFICER), asyncRoute(async (req, res) => {
    ...
    if (!driver)
        throw Object.assign(new Error('Driver not found'), { status: 404 });
    res.json(await db.driver.update({ where: { id: driver.id }, data: { onboardingStatus: client_1.OnboardingStatus.REJECTED, status: client_1.DriverStatus.OFF_DUTY, reviewNote } }));
}));
