import type { Place } from '../constants/routePlanning';

export type TollVehicleClass='CAR_VAN'|'LCV'|'BUS_TRUCK'|'MULTI_AXLE';
export type HistoricalTollObservation={source:string;destination:string;distanceKm:number;tollAmountInr:number;vehicleClass:TollVehicleClass;observedAt:Date|string};
export type HistoricalTollEstimate={estimatedTollInr:number;ratePerKmInr:number;source:'HISTORICAL_CORRIDOR'|'HISTORICAL_VEHICLE_CLASS'|'HISTORICAL_FLEET_NORMALIZED';confidence:'LOW'|'MEDIUM'|'HIGH';sampleSize:number;asOf:string};
export type TollHistoryTrip={id:string;vehicleId:string;vehicleType:string;vehicleCapacityKg:number;source:string;destination:string;distanceKm:number;createdAt:Date;dispatchedAt:Date|null;completedAt:Date|null;providerEstimatedTollInr:number|null};
export type TollHistoryExpense={vehicleId:string;amountInr:number;date:Date};

export function resolveTollVehicleClass(vehicleType:string,capacityKg:number):TollVehicleClass{
  const type=vehicleType.trim().toLowerCase();
  if(/\b(van|car|jeep)\b/.test(type))return 'CAR_VAN';
  if(/\b(lcv|light|mini)\b/.test(type)||(/\btruck\b/.test(type)&&capacityKg<=7500))return 'LCV';
  if(/\b(bus|coach|truck|hcv|heavy commercial)\b/.test(type))return capacityKg>16000?'MULTI_AXLE':'BUS_TRUCK';
  return capacityKg<=3500?'CAR_VAN':capacityKg<=7500?'LCV':capacityKg<=16000?'BUS_TRUCK':'MULTI_AXLE';
}

const normalized=(value:string)=>value.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
function placeTokens(place:Place){const specific=[place.name,place.city].filter((value):value is string=>Boolean(value&&value.trim().length>=3)).map(normalized);return [...new Set(specific.length?specific:[normalized(place.state)])]}
function containsPlace(value:string,place:Place){const text=normalized(value);return placeTokens(place).some(token=>text.includes(token))}
function sameCorridor(row:HistoricalTollObservation,source:Place,destination:Place){
...
export function estimateHistoricalToll(input:{source:Place;destination:Place;distanceKm:number;vehicleClass:TollVehicleClass;observations:HistoricalTollObservation[]}):HistoricalTollEstimate|undefined{
  ...
  const classFactors:Record<TollVehicleClass,number>={CAR_VAN:1,LCV:1.65,BUS_TRUCK:3.45,MULTI_AXLE:5.4};
  ...
  const source=normalized?'HISTORICAL_FLEET_NORMALIZED' as const:corridor.length?'HISTORICAL_CORRIDOR' as const:'HISTORICAL_VEHICLE_CLASS' as const;
...
export function buildHistoricalTollObservations(trips:TollHistoryTrip[],expenses:TollHistoryExpense[]):HistoricalTollObservation[]{
  return trips.flatMap(trip=>{
    ...
    if(tollAmountInr===null||tollAmountInr<=0||trip.distanceKm<=0)return [];
