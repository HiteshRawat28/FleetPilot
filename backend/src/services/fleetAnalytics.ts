export type AnalyticsVehicle={id:string;name:string;registrationNo:string;type:string;region:string;status:string;acquisitionCost:number};
export type AnalyticsFuel={vehicleId:string;cost:number;liters:number;date:Date};
export type AnalyticsMaintenance={vehicleId:string;cost:number;startDate:Date};
export type AnalyticsExpense={vehicleId:string;amount:number;type:string;date:Date};
export type AnalyticsTrip={vehicleId:string;revenue:number;plannedDistanceKm:number;fuelConsumedL:number|null;status:string;createdAt:Date;completedAt:Date|null};

type FleetAnalyticsInput={vehicles:AnalyticsVehicle[];fuel:AnalyticsFuel[];maintenance:AnalyticsMaintenance[];expenses:AnalyticsExpense[];trips:AnalyticsTrip[];now?:Date};

const sum=<T>(rows:T[],value:(row:T)=>number)=>rows.reduce((total,row)=>total+value(row),0);
const round=(value:number,digits=2)=>Number(value.toFixed(digits));
const monthKey=(date:Date)=>`${date.getUTCFullYear()}-${String(date.getUTCMonth()+1).padStart(2,'0')}`;
const monthLabel=(date:Date)=>new Intl.DateTimeFormat('en-IN',{month:'short',year:'2-digit',timeZone:'UTC'}).format(date);

export function calculateFleetAnalytics({vehicles,fuel,maintenance,expenses,trips,now=new Date()}:FleetAnalyticsInput){
  ...
  const expenseByType=(type:string)=>sum(expenses.filter(row=>row.type===type),row=>row.amount);
