...
type ToolName = 'get_fleet_summary'|'search_vehicles'|'search_drivers'|'search_trips'|'get_maintenance'|'get_finance_summary'|'get_analytics'|'check_assignment';
...
const allowedByTool:Record<ToolName,ChatRole[]> = {
  ...
  get_analytics: Object.values(Role),
  check_assignment: [Role.FLEET_MANAGER,Role.DISPATCHER]
};

export function toolNamesForRole(role:ChatRole):ToolName[]{
  return (Object.keys(allowedByTool) as ToolName[]).filter(name=>elevated.has(role)||allowedByTool[name].includes(role));
}
...
async function executeTool(db:PrismaClient,user:ChatUser,name:ToolName,raw:Record<string,unknown>):Promise<{data:unknown;evidence:Evidence}>{
  ...
  if(name==='get_fleet_summary'){
    ...
    return {data,evidence:evidence(name,'Fleet snapshot',`${vehicles.reduce((s,x)=>s+x._count,0)} vehicles · ${trips.find(x=>x.status===TripStatus.DISPATCHED)?._count||0} active trips`,data.recentTrips.map(t=>({label:t.tripNo,detail:`${t.route} · ${t.vehicle}`,status:t.status})))};
  }
  if(name==='search_vehicles'){
    ...
    const rows=await db.vehicle.findMany({where:{organizationId,...(dispatcher?{status:VehicleStatus.AVAILABLE}:requested?{status:requested}:{}),...(query?{OR:[{name:{contains:query,mode:'insensitive'}},{registrationNo:{contains:query,mode:'insensitive'}}]}:{})},take:clampLimit(raw.limit),orderBy:{name:'asc'}});
  ...
  if(name==='get_finance_summary'){
    ...
    const [fuel,expenses]=await Promise.all([db.fuelLog.findMany({where:{organizationId,date:{gte:since}},include:{vehicle:{select:{name:true}}}}),db.expense.findMany({where:{organizationId,date:{gte:since}},include:{vehicle:{select:{name:true}}}})]);const fuelCost=fuel.reduce((s,x)=>s+x.cost,0),otherCost=expenses.reduce((s,x)=>s+x.amount,0),liters=fuel.reduce((s,x)=>s+x.liters,0);
  ...
  if(name==='get_analytics'){
    const [vehicles,fuel,maintenance,expenses,trips]=await Promise.all([db.vehicle.findMany({where:{organizationId}}),db.fuelLog.findMany({where:{organizationId}}),db.maintenance.findMany({where:{organizationId}}),db.expense.findMany({where:{organizationId}}),db.trip.findMany({where:{organizationId}})]);const totalFuel=fuel.reduce((s,x)=>s+x.cost,0),totalMaintenance=maintenance.reduce((s,x)=>s+x.cost,0),totalOther=expenses.reduce((s,x)=>s+x.amount,0),liters=fuel.reduce((s,x)=>s+x.liters,0),distance=trips.filter(x=>x.status===TripStatus.COMPLETED).reduce((s,x)=>s+x.plannedDistanceKm,0),revenue=trips.reduce((s,x)=>s+x.revenue,0),acquisition=vehicles.reduce((s,x)=>s+x.acquisitionCost,0),active=vehicles.filter(x=>x.status!==VehicleStatus.RETIRED).length;
  ...
  if(vehicles.length!==1||drivers.length!==1){const data={eligible:false,needsClarification:true,vehicleMatches:vehicles.map(v=>`${v.name} (${v.registrationNo})`),driverMatches:drivers.map(d=>`${d.name} (${d.licenseNo})`)};return{data,evidence:evidence(name,'Assignment check','A unique vehicle and driver could not be identified')}}
  const vehicle=vehicles[0],driver=drivers[0],context=await assignmentContext(db,organizationId,vehicle.id,driver.id);let reasons:unknown[]=[];try{assertAssignmentEligible({...context,cargoWeightKg})}catch(error){if(error instanceof AssignmentEligibilityError)reasons=error.reasons;else throw error}const eligible=reasons.length===0;const data={eligible,vehicle:{id:vehicle.id,name:vehicle.name,registrationNo:vehicle.registrationNo},driver:{id:driver.id,name:driver.name,licenseNo:driver.licenseNo},cargoWeightKg,reasons};
  return {data,evidence:evidence(name,'Assignment check',eligible?'Vehicle and driver are eligible':'Assignment has conflicts',[{label:`${vehicle.name} + ${driver.name}`,detail:eligible?`${cargoWeightKg.toLocaleString('en-IN')} kg can be assigned`:(reasons as Array<{message:string}>).map(r=>r.message).join(' '),status:eligible?'ELIGIBLE':'CONFLICT'}])};
}

type GroqResponse={output?:Array<Record<string,any>>;output_text?:string;error?:{message?:string}};
